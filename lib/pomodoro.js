var growl = require('growl'),
  ...
  play = require('justaudio');
  

var items = {todo: [], completed:[]};
//var pomo_start = new Player('../sounds/pomo_start.mp3');
//var pomo_tick = new Player('../sounds/pomo_tick.mp3');
//var pomo_ring = new Player('../sounds/pomo_ring.mp3');

var Pomodoro = function(){};

Pomodoro.prototype.run = function(message, sound){
  var imageLocation = __dirname + "/../images/twentyfive.jpg";
  var TWENTYFIVE = 1000*60*25;
  var startTime = Date.now();
  play.playLocal('../sounds/pomo_ring.mp3');
  var running = setInterval(function () {
	if (sound) {
		//todo play a tick sound
	}
    var now = Date.now();
    var minsLeft = (((startTime + TWENTYFIVE - now)/1000)/60).toFixed(0);
    if (minsLeft < 5){
      log(("Minutes Left " + minsLeft).red);
    } else {
      log(("Minutes Left " + minsLeft).green);
    }
    if (now >= startTime + TWENTYFIVE){
      if (sound) {
    	//todo play a ring sound
      }    	
